...
> **Implementation status — v0.2–v0.7 (2026-07-20).** The shipped build deliberately diverges from parts of this spec; where they conflict, the following **supersedes** the text below:
...
> 8. **Command results in Méthodologie (v0.6).** `AppState` gains a persisted `results` map keyed by **stepId** (`{ [stepId]: string }`, like `checks`/`openSteps`), stored in the same `kv` blob (no schema change). Each step's expandable panel exposes a « Résultat » textarea to save captured command output; the field is part of the import/export contract. Deleting a step/phase/roadmap purges its result, but a progress reset keeps it (results are data, not progression). OPSEC: output is stored cleartext at rest like other free text.
> 9. **Multiple profiles (v0.7).** The single dataset becomes **one of many named profiles** (OSCP, RT, per-client…). A profile **is one `AppState`**, stored as a JSON blob in a new `profiles` row (`{id, name, position, state}`); this supersedes the relational per-entity storage of §12 for content (the old tables are retained only to migrate an existing DB once, on first start, into a profile named **OSCP** — transactional, non-destructive). The API becomes per-profile: `GET/POST /api/profiles`, `PUT/DELETE /api/profiles/:id`, `POST /api/profiles/:id/activate`, `GET/PUT /api/profiles/:id/state`; `import`/`export` act on the active profile; the former global `GET/PUT /api/state` is removed. A new profile is empty or a **clone** of an existing one; single commands/references/whole methodologies can be **copied across profiles** with fresh ids (a command brings its category + note; a methodology is copied structure-only). Variable **values** remain global/session and **unchanged across profile switches** (D7 preserved) — a profile scopes content, not values. This finally lands the "profiles layer with no value-schema migration" foreseen at §5.1/D2/Open Items, generalized from value-profiles to full content-profiles.
